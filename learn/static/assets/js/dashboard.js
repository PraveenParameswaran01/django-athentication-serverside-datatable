...
function datatablebind() {
  dataTable = $('#userslistdatatableId').DataTable({
    responsive: true,
    pagingType: "full",
    order: [[1, 'desc']],
    "aLengthMenu": [
    ...
    serverMethod: 'GET',
    "ajax": {
      "url": '/api/users_list', // Update the URL with the correct endpoint
      'data': function (data) { }
    },
    'columns': [
      {
        "data": null, orderable: false, "class": "text-center", 'width': 20, render: function (data, type, row, meta) {
          count = (meta.row + 1) + (meta.settings._iDisplayStart)
          return `<span class="circle-bg">${count}</span>`
        }
      },
      {
